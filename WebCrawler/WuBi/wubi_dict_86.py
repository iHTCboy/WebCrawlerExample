...
def save_newJson(dict_json, file_name=None):
...
Dic_86 = {}
for key, value in wubi_86_dict.items():
    str = chr(key)
    Dic_86[str] = value.split(',')
    Dic_86[str].sort(key=lambda x: len(x), reverse=False)
    #print(Dic_86)
...
Dic_98 = defaultdict(list)

def generate_single_word_dict(fpath):
    ...
    for line in f:
        ...
        if len(word) == 1:
            Dic_98[word].append(code)
    f.close()

    print(Dic_98)

    for word in Dic_98:
        Dic_98[word].sort(key=lambda x: len(x), reverse=False)
